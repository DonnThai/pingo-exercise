class Admin::ShoesController < Admin::AdminController
  ...
  # POST /shoes.json
  def create
    Rails.logger.info "[create shoe from admin], #{params.inspect}"
    @shoe = Shoe.new(shoe_params)

    respond_to do |format|
      if @shoe.save
        format.html { redirect_to admin_shoes_url(@shoe), notice: 'Shoe was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @shoe.update(shoe_params)
        format.html { redirect_to admin_shoes_url(@shoe), notice: 'Shoe was successfully updated.' }
